...
aws iam create-access-key --user-name $AWS_USR | tee tmp$$.log
...
echo "Automating the above: creating kops AWS profile..."
AKI=`cat tmp$$.log | jq '.AccessKey.AccessKeyId' | tr -d \"`
SAK=`cat tmp$$.log | jq '.AccessKey.SecretAccessKey' | tr -d \"`
...
# Make sure you clean-up log containing sensitive credentials
rm -f tmp$$.log
